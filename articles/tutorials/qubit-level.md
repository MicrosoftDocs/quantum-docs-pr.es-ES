...
<span data-ttu-id="b466b-110">En nuestro caso, vamos a definir una Q# operación para realizar la transformación completa de qubit de Quantum de tres-, que tiene la siguiente representación como un circuito:</span><span class="sxs-lookup"><span data-stu-id="b466b-110">In our case, we will define a Q# operation to perform the full three-qubit quantum Fourier transform, which has the following representation as a circuit:</span></span>
...
> [!div class="checklist"]
> * <span data-ttu-id="b466b-115">Definir operaciones Quantum en Q#</span><span class="sxs-lookup"><span data-stu-id="b466b-115">Define quantum operations in Q#</span></span>
> * <span data-ttu-id="b466b-116">Llamar a Q# operaciones directamente desde el símbolo del sistema o mediante un programa host clásico</span><span class="sxs-lookup"><span data-stu-id="b466b-116">Call Q# operations directly from the command prompt or using a classical host program</span></span>
...
1. <span data-ttu-id="b466b-120">El propio programa, que se implementa mediante el Q# lenguaje de programación Quantum, y, a continuación, se invoca para ejecutarse en un equipo Quantum o en un simulador Quantum.</span><span class="sxs-lookup"><span data-stu-id="b466b-120">The program itself, which is implemented using the Q# quantum programming language, and then invoked to run on a quantum computer or quantum simulator.</span></span> <span data-ttu-id="b466b-121">Se componen de</span><span class="sxs-lookup"><span data-stu-id="b466b-121">These consist of</span></span> 
    - <span data-ttu-id="b466b-122">Q# operaciones: subrutinas que actúan sobre registros Quantum y</span><span class="sxs-lookup"><span data-stu-id="b466b-122">Q# operations: subroutines acting on quantum registers, and</span></span> 
    - <span data-ttu-id="b466b-123">Q# funciones: subrutinas clásicas que se usan en el algoritmo Quantum.</span><span class="sxs-lookup"><span data-stu-id="b466b-123">Q# functions: classical subroutines used within the quantum algorithm.</span></span>
...
<span data-ttu-id="b466b-128">La primera parte de este tutorial consiste en definir la Q# operación `Perform3qubitQFT` , que realiza la transformación de Fourier de Quantum en tres qubits.</span><span class="sxs-lookup"><span data-stu-id="b466b-128">The first part of this tutorial consists of defining the Q# operation `Perform3qubitQFT`, which performs the quantum Fourier transform on three qubits.</span></span> 
...
<span data-ttu-id="b466b-130">El primer paso es crear el Q# proyecto y el archivo.</span><span class="sxs-lookup"><span data-stu-id="b466b-130">The first step is creating your Q# project and file.</span></span>
...
### <a name="namespaces-to-access-other-no-locq-operations"></a><span data-ttu-id="b466b-133">Espacios de nombres para tener acceso a otras Q# operaciones</span><span class="sxs-lookup"><span data-stu-id="b466b-133">Namespaces to access other Q# operations</span></span>
...
<span data-ttu-id="b466b-135">Para que nuestra operación haga uso de Q# las operaciones existentes, abrimos los `Microsoft.Quantum.<>` espacios de nombres pertinentes.</span><span class="sxs-lookup"><span data-stu-id="b466b-135">For our operation to make use of existing Q# operations, we open the relevant `Microsoft.Quantum.<>` namespaces.</span></span>
...
<span data-ttu-id="b466b-141">Dentro de nuestra Q# operación, asignamos primero un registro de tres qubits con la `using` instrucción:</span><span class="sxs-lookup"><span data-stu-id="b466b-141">Within our Q# operation, we first allocate a register of three qubits with the `using` statement:</span></span>
...
<span data-ttu-id="b466b-149">Q# ya contiene muchas de las puertas de Quantum básicas como operaciones en el [`Microsoft.Quantum.Intrinsic`](xref:Microsoft.Quantum.Intrinsic) espacio de nombres y no son excepciones.</span><span class="sxs-lookup"><span data-stu-id="b466b-149">Q# already contains many basic quantum gates as operations in the [`Microsoft.Quantum.Intrinsic`](xref:Microsoft.Quantum.Intrinsic) namespace, and these are no exception.</span></span> 

<span data-ttu-id="b466b-150">Dentro de una Q# operación, las instrucciones que invocan las llamadas a, por supuesto, se ejecutarán en orden secuencial.</span><span class="sxs-lookup"><span data-stu-id="b466b-150">Within a Q# operation, the statements invoking callables will, of course, be run in sequential order.</span></span>
...
<span data-ttu-id="b466b-157">Q# facilita en gran medida la ejecución de una operación en uno o varios qubits de control.</span><span class="sxs-lookup"><span data-stu-id="b466b-157">Q# makes it extremely easy to condition the run of an operation upon one or multiple control qubits.</span></span>
...
<span data-ttu-id="b466b-169">Por lo tanto, hemos terminado de escribir las operaciones de nivel de qubit de la transformación Fourier de Quantum en nuestra Q# operación:</span><span class="sxs-lookup"><span data-stu-id="b466b-169">Hence we have finished writing the qubit-level operations of the quantum Fourier transform into our Q# operation:</span></span>
...
<span data-ttu-id="b466b-171">Nuestro qubits estaba en el estado $ \ket {0} $ cuando lo hemos asignado y, en gran parte, en nuestra vida, deberíamos Q# dejar cosas de la misma manera en que las encontramos (o mejor).</span><span class="sxs-lookup"><span data-stu-id="b466b-171">Our qubits were in state $\ket{0}$ when we allocated them, and much like in life, in Q# we should leave things the same way we found them (or better!).</span></span>
...
<span data-ttu-id="b466b-177">El Q# archivo completo debe tener el siguiente aspecto:</span><span class="sxs-lookup"><span data-stu-id="b466b-177">Your full Q# file should now look like this:</span></span>
...
<span data-ttu-id="b466b-178">Una Q# vez completado el archivo y la operación, el programa Quantum está listo para ser llamado y simulado.</span><span class="sxs-lookup"><span data-stu-id="b466b-178">With the Q# file and operation complete, our quantum program is ready to be called and simulated.</span></span>
...
<span data-ttu-id="b466b-180">Una vez definida Q# la operación en un `.qs` archivo, ahora es necesario llamar a esa operación y observar cualquier dato clásico devuelto.</span><span class="sxs-lookup"><span data-stu-id="b466b-180">Having defined our Q# operation in a `.qs` file, we now need to call that operation and observe any returned classical data.</span></span>
<span data-ttu-id="b466b-181">Por ahora, no se devuelve nada (Recuerde que la operación definida anteriormente devuelve `Unit` ), pero, cuando se modifica posteriormente la Q# operación para devolver una matriz de resultados de la medición ( `Result[]` ), se solucionará este error.</span><span class="sxs-lookup"><span data-stu-id="b466b-181">For now, there isn't anything returned (recall that our operation defined above returns `Unit`), but when we later modify the Q# operation to return an array of measurement results (`Result[]`), we will address this.</span></span>

<span data-ttu-id="b466b-182">Aunque el Q# programa está omnipresente en todos los entornos que se usan para llamarlo, la manera de hacerlo será diferente.</span><span class="sxs-lookup"><span data-stu-id="b466b-182">While the Q# program is ubiquitous across the environments used to call it, the manner of doing so will of course vary.</span></span> <span data-ttu-id="b466b-183">Como tal, simplemente siga las instrucciones de la pestaña correspondiente a su configuración: trabajar desde la Q# aplicación o mediante un programa host en Python o C#.</span><span class="sxs-lookup"><span data-stu-id="b466b-183">As such, simply follow the instructions in the tab corresponding to your setup: working from the Q# application or using a host program in Python or C#.</span></span>
...
<span data-ttu-id="b466b-185">Ejecutar el Q# programa desde el símbolo del sistema solo requiere un pequeño cambio en el Q# archivo.</span><span class="sxs-lookup"><span data-stu-id="b466b-185">Running the Q# program from the command prompt requires only a small change to the Q# file.</span></span>
...
1. <span data-ttu-id="b466b-192">En primer lugar, se importa el `qsharp` módulo, que registra el cargador de módulos para la Q# interoperabilidad.</span><span class="sxs-lookup"><span data-stu-id="b466b-192">First, we import the `qsharp` module, which registers the module loader for Q# interoperability.</span></span> 
    <span data-ttu-id="b466b-193">Esto permite Q# que los espacios de nombres (por ejemplo, el que `NamespaceQFT` hemos definido en nuestro Q# archivo) aparezcan como módulos de Python, desde los que se pueden importar Q# operaciones.</span><span class="sxs-lookup"><span data-stu-id="b466b-193">This allows Q# namespaces (e.g. the `NamespaceQFT` we defined in our Q# file) to appear as Python modules, from which we can import Q# operations.</span></span>
2. <span data-ttu-id="b466b-194">A continuación, importe las Q# operaciones que llamaremos directamente---en este caso, `Perform3qubitQFT` .</span><span class="sxs-lookup"><span data-stu-id="b466b-194">Then, import the Q# operations which we will directly invoke---in this case, `Perform3qubitQFT`.</span></span>
    <span data-ttu-id="b466b-195">Solo es necesario importar el punto de entrada en un Q# programa (es decir, _no_ operaciones como `H` y `R1` , que son llamadas por otras Q# operaciones, pero nunca por el host clásico).</span><span class="sxs-lookup"><span data-stu-id="b466b-195">We need only import the entry point into a Q# program (i.e. _not_ operations like `H` and `R1`, which are called by other Q# operations but never by the classical host).</span></span>
3. <span data-ttu-id="b466b-196">En simulación de Q# operaciones o funciones, use el formulario `<Q#callable>.simulate(<args>)` para ejecutarlas en el `QuantumSimulator()` equipo de destino.</span><span class="sxs-lookup"><span data-stu-id="b466b-196">In simulating Q# operations or functions, use the form `<Q#callable>.simulate(<args>)` to run them on the `QuantumSimulator()` target machine.</span></span> 

> [!NOTE]
> <span data-ttu-id="b466b-197">Si deseamos llamar a la operación en otro equipo, por ejemplo `ResourceEstimator()` , simplemente usaremos `<Q#callable>.estimate_resources(<args>)` .</span><span class="sxs-lookup"><span data-stu-id="b466b-197">If we wanted to call the operation on a different machine, for example the `ResourceEstimator()`, we would simply use `<Q#callable>.estimate_resources(<args>)`.</span></span>
> <span data-ttu-id="b466b-198">En general, Q# las operaciones son independientes de las máquinas en las que se ejecutan, pero algunas características como `DumpMachine` pueden comportarse de forma diferente.</span><span class="sxs-lookup"><span data-stu-id="b466b-198">In general, Q# operations are agnostic to the machines on which they're run, but some features such as `DumpMachine` may behave differently.</span></span>

4. <span data-ttu-id="b466b-199">Tras realizar la simulación, la llamada a la operación devolverá valores tal como se define en el Q# archivo.</span><span class="sxs-lookup"><span data-stu-id="b466b-199">Upon performing the simulation, the operation call will return values as defined in the Q# file.</span></span>
...
3. <span data-ttu-id="b466b-211">Ejecute el algoritmo cuántico.</span><span class="sxs-lookup"><span data-stu-id="b466b-211">Run the quantum algorithm.</span></span> 
    <span data-ttu-id="b466b-212">Cada Q# operación genera una clase de C# con el mismo nombre.</span><span class="sxs-lookup"><span data-stu-id="b466b-212">Each Q# operation generates a C# class with the same name.</span></span> 
...
<span data-ttu-id="b466b-249">Para implementar las medidas dentro de un Q# programa, usamos la `M` operación (from `Microsoft.Quantum.Intrinsic` ), que devuelve un `Result` tipo.</span><span class="sxs-lookup"><span data-stu-id="b466b-249">To implement measurements within a Q# program, we use the `M` operation (from `Microsoft.Quantum.Intrinsic`), which returns a `Result` type.</span></span>
...
> [!NOTE]
> <span data-ttu-id="b466b-259">La sintaxis de esta instrucción es única para Q# , pero corresponde a la reasignación de variables similar que se ha `resultArray[i] <- M(qs[i])` detectado en otros lenguajes, como F # y R.</span><span class="sxs-lookup"><span data-stu-id="b466b-259">The syntax of this statement is unique to Q#, but corresponds to the similar variable reassignment `resultArray[i] <- M(qs[i])` seen in other languages such as F# and R.</span></span>
...
## <a name="use-the-no-locq-libraries"></a><span data-ttu-id="b466b-292">Uso de las Q# bibliotecas</span><span class="sxs-lookup"><span data-stu-id="b466b-292">Use the Q# libraries</span></span>
<span data-ttu-id="b466b-293">Tal y como se mencionó en la introducción, gran parte de la Q# capacidad de los demás tiene en cuenta que le permite abstraer las preocupaciones de trabajar con qubits individuales.</span><span class="sxs-lookup"><span data-stu-id="b466b-293">As we mentioned in the introduction, much of Q#'s power rests in the fact that it allows you to abstract-away the worries of dealing with individual qubits.</span></span>
...
<span data-ttu-id="b466b-295">Las Q# bibliotecas contienen la operación [QFT](xref:Microsoft.Quantum.Canon.QFT) , que se puede realizar simplemente y aplicar para cualquier número de qubits.</span><span class="sxs-lookup"><span data-stu-id="b466b-295">The Q# libraries contain the [QFT](xref:Microsoft.Quantum.Canon.QFT) operation, which you can simply take and apply for any number of qubits.</span></span>
<span data-ttu-id="b466b-296">Para probarlo, defina una nueva operación en el Q# archivo que tenga el mismo contenido de `Perform3QubitQFT` , pero con todo lo que haya entre el primero `H` y el `SWAP` reemplazado por dos líneas sencillas:</span><span class="sxs-lookup"><span data-stu-id="b466b-296">To give it a try, define a new operation in your Q# file which has the same contents of `Perform3QubitQFT`, but with everything from the first `H` to the `SWAP` replaced by two easy lines:</span></span>
```qsharp
...
<span data-ttu-id="b466b-299">Para tener acceso a estas operaciones, agregue `open` instrucciones para sus respectivos espacios de nombres al principio del Q# archivo:</span><span class="sxs-lookup"><span data-stu-id="b466b-299">To have access to these operations, add `open` statements for their respective namespaces at the beginning of the Q# file:</span></span>
```qsharp
...
<span data-ttu-id="b466b-301">Para ver las ventajas reales del uso de las Q# operaciones de biblioteca, cambie el número de qubits a algo distinto de `3` :</span><span class="sxs-lookup"><span data-stu-id="b466b-301">To see the real benefit of using the Q# library operations, change the number of qubits to something other than `3`:</span></span>
```qsharp
