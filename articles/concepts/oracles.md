...
En segundo lugar, aunque $n = m $, la función no se puede invertir: si $f (x) = f (y) $ para algunos $x \ne y $, entonces $O \ket{x} = O\ket {y} $ pero $O ^ \dagger O\ket {x} \ne O ^ \dagger O\ket {y} $.
...
Lo importante es que la definición de Oracle de este modo para cada estado de base de cálculo $ \ket{x}\ket{y} $ también define cómo $O $ actúa para cualquier otro Estado.
...
Como alternativa, podemos codificar $f $ en un Oracle $O $ aplicando una _fase_ basada en la entrada a $O $.
