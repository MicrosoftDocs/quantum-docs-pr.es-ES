...
En la biblioteca QML, la mejor manera de modificar los hiperparámetros es invalidar los valores predeterminados del [`TrainingOptions`](xref:microsoft.quantum.machinelearning.trainingoptions)UDT. Para ello, se llama con la función [`DefaultTrainingOptions`](xref:microsoft.quantum.machinelearning.defaulttrainingoptions) y se aplica el operador `w/` para invalidar los valores predeterminados. Por ejemplo, para usar medidas 100.000 y una velocidad de aprendizaje de 0,01:
 ```qsharp
