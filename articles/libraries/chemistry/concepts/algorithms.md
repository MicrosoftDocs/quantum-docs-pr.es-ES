...
Deje que $r $ sea el número de pasos que se llevan a cabo en la evolución de tiempo, por lo que cada paso de tiempo se ejecuta para el tiempo $t/r $. Después, tenemos $ $ e ^ {-i \ sum_ {j = 1} ^ m H_j t} = \left (\ prod_ {j = 1} ^ m e ^ {-iH_j t/r} \ right) ^ r + O (m ^ 2 t ^ 2/r). $ $, que implica que si $r $ se escala como $m ^ 2 t ^ 2/\ épsilon $, el error se puede hacer como máximo $ \epsilon $ para cualquier $ \epsilon > 0 $.
...
Los exponenciales de los operadores Pauli se pueden implementar directamente en Q # mediante la operación <xref:microsoft.quantum.intrinsic.exp>:
```qsharp
...
El paso siguiente implica transformar el vectores propios del operador de recorrido de $e ^ {i\pm \cos ^{-1}(E_k/| h | _ 1)} $, donde $E _k $ son los vectores propios de $H $ a $e ^ {-iE_k t} $.
...
Mientras que la operación de selección proporciona un medio para tener acceso coherente a cada uno de los términos de Hamiltonian $H _j $ la subrutina de preparación proporciona un método para obtener acceso a los coeficientes $h _j $, \begin{Equation} \operatorname{Prepare}\ket{0} = \ sum_j \sqrt{\frac{h_j} {| H | _ 1}} \ket{j}.
