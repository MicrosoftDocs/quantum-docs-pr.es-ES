...
Por ejemplo, `(7)` es `Int` una expresión, `([1,2,3])` es una expresión de tipo matriz de `Int`s y `((1,2))` es una expresión con tipo `(Int, Int)`.

La equivalencia entre los valores simples y las tuplas de un solo elemento que se describen en [el modelo](xref:microsoft.quantum.language.type-model#tuple-types) de tipo `(6)` elimina la ambigüedad entre `(6)` como un grupo y como una tupla de un solo elemento.

## <a name="symbols"></a>Símbolos

El nombre de un símbolo enlazado o asignado a un valor de `'T` tipo es una expresión de `'T`tipo.
Por ejemplo, si el símbolo `count` se enlaza al valor `5`entero, `count` es una expresión de entero.

## <a name="numeric-expressions"></a>Expresiones numéricas

Las expresiones numéricas son expresiones `Int`de `BigInt`tipo, `Double`o.
Es decir, son números enteros o de punto flotante.

`Int`los literales en Q # son idénticos a los literales enteros en C#, excepto en que no se requiere (o se permite) ninguna "l" o "L" final.
Los enteros hexadecimales y binarios se admiten con un prefijo "0x" y "0B", respectivamente.

`BigInt`los literales en Q # son idénticos a cadenas de tipo entero Big en .NET, con una "l" o "L" al final.
Los enteros Big hexadecimales se admiten con un prefijo "0x".
Por lo tanto, los siguientes son usos válidos de `BigInt` los literales:
...
`Double`los literales en Q # son idénticos a los literales Double en C#, excepto en que no se requiere (o se permite) ninguna "d" o "D" final.

Dada una expresión de matriz de cualquier tipo de elemento `Int` , una expresión se puede formar `Length` utilizando la función integrada, con la expresión de matriz entre paréntesis, `(` y. `)`
Por ejemplo, si `a` está enlazado a una matriz, `Length(a)` es una expresión de entero.
Si `b` es una matriz de matrices de enteros `Int[][]`,, entonces `Length(b)` es el número de submatrices `b`de y `Length(b[1])` es el número de enteros de la segunda submatriz de. `b`
...
Si el número que se va a desplazar es un entero, se interpreta `mod 64`la cantidad de desplazamiento. es decir, un desplazamiento de 1 y un turno de 65 tienen el mismo efecto.
...
Es decir, `a % b` siempre tendrá el mismo signo `a`que y `b * (a / b) + a % b` siempre será igual `a`que.
Por ejemplo:
...
Los dos `Bool` valores literales `true` son `false`y.

Dadas dos expresiones cualesquiera del mismo tipo primitivo, se `==` pueden `!=` usar los operadores binarios y para construir `Bool` una expresión.
La expresión será true si las dos expresiones son iguales y false en caso contrario.

No se pueden comparar los valores de los tipos definidos por el usuario, solo se pueden comparar los valores desencapsulados. Por ejemplo, mediante el operador `!` "desencapsular" (que se explica en la [Página modelo de Q # type](xref:microsoft.quantum.language.type-model#user-defined-types)),
...
La comparación de `Qubit` igualdad para los valores es la igualdad de identidad; es decir, si las dos expresiones identifican el mismo qubit.
Esta comparación no compara, tiene acceso, mide o modifica el estado de los dos qubits.

La comparación de `Double` igualdad de los valores puede ser engañosa debido a efectos de redondeo.
...
Dadas dos expresiones booleanas, se `and` pueden `or` usar los operadores binarios y para construir una nueva expresión booleana que es true si las dos expresiones son verdaderas (o ambas).
...
Q # permite usar cadenas en la `fail` instrucción y en la `Log` función estándar.
...
La sintaxis de Q # para las interpolaciones de cadenas es un subconjunto de la sintaxis de C# 7,0; Q # no admite cadenas interpoladas textuales (multilínea).
Vea [*cadenas interpoladas*](https://docs.microsoft.com/dotnet/csharp/language-reference/keywords/interpolated-strings) para la sintaxis de C#.
...
Las únicas `Qubit` expresiones son símbolos que se enlazan `Qubit` a valores o elementos de `Qubit` matriz de matrices.
...
Aparte de eso, las únicas `Pauli` expresiones son símbolos que se enlazan `Pauli` a valores o elementos de `Pauli` matriz de matrices.

## <a name="result-expressions"></a>Expresiones de resultado

Los dos `Result` valores, `One` y `Zero`, son expresiones `Result` válidas.

Aparte de eso, las únicas `Result` expresiones son símbolos que se enlazan `Result` a valores o elementos de `Result` matriz de matrices.
En concreto, tenga en `One` cuenta que no es igual que el entero `1`y no hay ninguna conversión directa entre ellos.
Lo mismo se cumple para `Zero` y `0`.

## <a name="range-expressions"></a>Expresiones de rango

Dadas las `Int` tres `start`expresiones `step`, y `stop`, `start .. step .. stop` es una expresión de rango cuyo primer elemento `start`es, el segundo `start+step`elemento es, el `start+step+step`tercer elemento es, etc `stop` ., hasta que se pasa.
Un intervalo puede estar vacío si, por ejemplo, `step` es positivo y `stop < start`.
El último elemento del intervalo `stop` será si la diferencia entre `start` y `stop` es un múltiplo entero de; `step` es decir, el intervalo es inclusivo en ambos extremos.

Dadas dos `Int` expresiones `start` cualesquiera `stop`y `start .. stop` , es una expresión de rango que es `start .. 1 .. stop`igual a.
...
Por ejemplo, `X` es un literal de operación que hace referencia a la `X` operación de la `Message` biblioteca estándar y es un literal de función que hace `Message` referencia a la función de la biblioteca estándar.

Si una operación admite el `Adjoint` functor, `Adjoint op` es una expresión de operación.
Del mismo modo, si la operación `Controlled` admite el functor `Controlled op` , es una expresión de operación.
Los tipos de estas expresiones se especifican en [funcs](xref:microsoft.quantum.language.type-model#functors).

Los funcrs`Adjoint` ( `Controlled`y) se enlazan más estrechamente que todos los demás operadores, salvo el `!` operador Unwrap y la `[]`indización de matriz con.
...
Del mismo modo `Sin` , si es una función `(Double -> Double)`con `Sin(0.1)` firma, es una expresión `Double`de tipo.
Por último, `Builder` si es una función con `(Int -> (Int -> Int))`firma, `Builder(3)` es una función de into a int.
...
Por lo tanto, para invocar el `Builder` resultado de llamar a desde el párrafo anterior, la sintaxis correcta es:

```qsharp
(Builder(3))(2)
```

Al invocar un parámetro de tipo invocable, los parámetros de tipo reales se pueden especificar entre `<` corchetes `>` angulares y después de la expresión que se puede llamar.
...
Por ejemplo, si `Func` tiene Signature `('T1, 'T2, 'T1) -> 'T2`, `Op1` y `Op2` tiene `(Qubit[] => Unit is Adj)`Signature, y `Op3` tiene Signature `(Qubit[] => Unit)`, para `Func` invocar `Op1` con como primer argumento, `Op2` como el segundo y `Op3` como el tercero:

```qsharp
let combinedOp = Func<(Qubit[] => Unit), (Qubit[] => Unit is Adj)>(Op1, Op2, Op3);
```

La especificación de tipo es necesaria `Op3` porque `Op1` y tienen tipos diferentes, por lo que el compilador lo tratará como ambiguo sin la especificación.
...
En Q #, una función de llamada parcialmente aplicada se expresa escribiendo una expresión de invocación normal, pero usando un carácter `_`de subrayado,, para los argumentos no especificados.
...
Por ejemplo, si `Op` tiene el `((Int, ((Qubit, Qubit), Double)) => Unit is Adj)`tipo:

- `Op(5,(_,_))`tiene el `(((Qubit,Qubit), Double) => Unit is Adj)`tipo y, por `Op(5,_)`tanto, tiene.
- `Op(_,(_,1.0))` tiene tipo `((Int, (Qubit,Qubit)) => Unit is Adj)`.
- `Op(_,((q1,q2),_))` tiene tipo `((Int,Double) => Unit is Adj)`.
...
Por ejemplo, si `Op` tiene el `(('T1, Qubit, 'T1) => Unit : Adjoint)`tipo:
...
Por ejemplo, si `IntPair` es un tipo definido por el usuario basado `(Int, Int)`en, `IntPair(2,3)` sería un literal válido de ese tipo.
...
Por ejemplo, si `IntPair` es un tipo definido por el usuario con el `(Int, Int)`tipo subyacente `s` y era una variable con `IntPair(2,3)`el valor `s!` , sería `(2,3)`.

Para los tipos definidos por el usuario definidos en términos de otros tipos definidos por el usuario. el operador Unwrap puede repetirse; por ejemplo, `s!!` indica el valor de doble desencapsulado de `s`.
Por lo tanto `WrappedPair` , si es un tipo definido por el usuario `IntPair`con el `t` tipo subyacente, y es `WrappedPair(IntPair(1,2))`una variable `t!!` con el `(1,2)`valor, entonces sería.
...
Por ejemplo, si `Op1`, `Op2`y `Op3` todos `Qubit[] => Unit`son, pero `Op1` admite `Adjoint` `Op2` `Controlled`, y `Op3` admite ambos:
...
En lugar de `new ★[0]`usar, `★` donde es un marcador de posición para un tipo adecuado, permite crear la matriz deseada de longitud cero.
...
Las matrices que contienen qubits o llamadas se deben inicializar correctamente con valores no predeterminados antes de que sus elementos se puedan usar de forma segura. Se pueden encontrar rutinas de inicialización <xref:microsoft.quantum.arrays>adecuadas en.
...
Dada una expresión de matriz y `Range` una expresión, se puede formar una nueva expresión mediante `[` el `]` operador de segmento de matriz y.
...
Por ejemplo, si `a` está enlazado a una matriz `Double`de s, `a[3..-1..0]` es una `Double[]` expresión que contiene los cuatro primeros elementos de `a` , pero en el orden inverso en el `a`que aparecen en.
...
Es decir, el primer elemento de una matriz `a` siempre `a[0]`es.
...
Dada una expresión de matriz y `Int` una expresión, se puede formar una nueva expresión mediante `[` el `]` operador de elemento de matriz y.
La nueva expresión será del mismo tipo que el tipo de elemento de la matriz.
Por ejemplo, si `a` está enlazado a una matriz `Double`de s, `a[4]` es una `Double` expresión.
...
Una expresión `expression1 w/ expression2 <- expression3`de copia y actualización es una expresión con el formato, donde `expression1` debe ser de tipo `T[]` para algún tipo. `T` La segunda `expression2` define los índices de los elementos que se van a modificar en comparación con la matriz de `expression1` y debe ser de tipo `Int` o de tipo. `Range` Si `expression2` es de tipo `Int`, `expression3` debe ser de tipo `T`. Si `expression2` es de tipo `Range`, `expression3` debe ser de tipo `T[]`.

Una expresión `arr w/ idx <- value` de copia y actualización crea una nueva matriz con todos los elementos establecidos en el elemento correspondiente de `arr`, excepto los elementos de `idx`, que se establecen en los elementos de `value`, que están establecidos en. Por ejemplo, si `arr` contiene una matriz `[0,1,2,3]`, 
...
Por ejemplo `a==b ? C(qs) | D(qs)`, en caso de que `a==b` sea true, se invocará la `C` operación y, si es false, solo `D` se invocará.
...
Los funcrs `Adjoint` y `Controlled` se enlazan después de la indización de matriz, pero antes de todos los demás operadores.
