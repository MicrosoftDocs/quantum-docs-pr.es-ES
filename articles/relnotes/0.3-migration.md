...
Nos complace presentar la actualización más reciente del kit de desarrollo de Quantum.  En esta página se proporciona información general sobre las nuevas características del lenguaje Q # y se describe cómo [migrar](#Migration) programas q # existentes a la nueva sintaxis.  Consulte la documentación completa para obtener una descripción completa [del lenguaje de](xref:microsoft.quantum.guide.basics)preguntas y respuestas.
...
Las asignaciones dentro de `using` y `borrowing` son nuevas posibles para una sola qubits, matrices qubit y tuplas anidadas:
```qsharp
    using (qubit = Qubit()) {
...
Se han incorporado dos nuevas directivas de generación de especializaciones y además de `invert` `distribute` `auto` para aumentar el control sobre cómo se generan las especializaciones de funcción.
Una declaración explícita de la `body` especialización predeterminada dentro de las operaciones ya no es obligatoria.
...
Los modificadores se enlazan de forma más estrecha que cualquier otro combinador de expresión y, en particular, más estrecho que el combinador de expresiones de llamada `(` `)` . 
`Adjoint`, `Controlled` y un nuevo operador postfijo "desencapsulado" `!` se trata como modificadores de expresión. Tratar `Adjoint` y `Controlled` como modificadores de expresión elimina la necesidad de paréntesis en muchos casos. Las expresiones siguientes, por ejemplo, son válidas para `Op` , una operación unitario, `opArr` una matriz de operaciones unitarios y `arg` un argumento adecuado: 
```qsharp
    Adjoint Op (arg);
...
```
Como se puede observar en el ejemplo, se enlaza con los `!` modificadores de prefijo, como `Controlled` y `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Cambios importantes 
...
Además de los cambios que se enumeran a continuación, el patrón `__*__` donde * es cualquier secuencia de caracteres que no sean espacios en blanco se reserva para uso interno y no se puede usar como nombre de símbolo.  
...
Un ejemplo de este tipo de cambio es la tupla de símbolos en las especializaciones definidas por el usuario. En las versiones anteriores, una declaración de especialización controlada o controlada por se especificaba un único argumento de símbolo, a partir de 0,3 se recomienda proporcionar una tupla de símbolos de este tipo para todas las especializaciones, `...` lo que indica que el compilador debe migrar los símbolos de la declaración a la que se puede llamar si es necesario. 

## <a name="semantic-changes"></a>Cambios semánticos 

La interrupción más impactante de la versión 0,3 es la forma en que los tipos definidos por el usuario se integran en el sistema de tipos. En versiones anteriores, los tipos definidos por el usuario se consideraban un subtipo de su tipo subyacente. Con esta versión, esto ya no es así. Los tipos definidos por el usuario ahora se consideran su propio tipo distinto y no existe ninguna conversión automática entre un tipo definido por el usuario y su tipo subyacente. Todas las conversiones deben hacerse explícitamente a través de un operador "Unwrap" `!` , y se necesita una conversión en particular para tener acceso al contenido de un objeto de tipo definido por el usuario.  
...
En el ejemplo siguiente se muestra cómo utilizar la `ToOperation` función para cambiar una función `Square` a una operación `op` : 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Migración de proyectos existentes a Q # 0,3 
...
2. En un editor de texto, edite el archivo. csproj para cambiar la versión de todos los "Microsoft. Quantum" `PackageReference` a la versión 0.3.1811.2802-Preview, por ejemplo:
```xml
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Script de migración 
...
- El proyecto se compila con la versión anterior del kit de desarrollo de Quantum para asegurarse de que es probable que la migración se realice correctamente. Este paso se puede omitir mediante el `-Force` parámetro en el script.
...
> Para permitir que los scripts como `qdk-migrate.ps1` se ejecuten en Windows, es posible que tenga que cambiar la Directiva de ejecución.
