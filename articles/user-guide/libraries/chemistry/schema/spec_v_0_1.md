...
La `version` propiedad debe tener el valor `"0.1"` .
...
Cada elemento del valor de la `integral_sets` propiedad debe ser un objeto JSON que describe un conjunto de enteros, como se describe en el resto de esta sección.
En el resto de esta sección, el término "objeto de conjunto entero" hará referencia a un elemento en el valor de la `integral_sets` propiedad del objeto Broombridge.
...
La `hamiltonian` propiedad de cada objeto de conjunto entero describe el valor de Hamiltonian para un problema de química de Quantum determinado enumerando sus términos de uno y dos cuerpos como matrices dispersas de números reales.
...
El objeto de representación de partícula-agujero especifica que los enteros almacenados se definen con respecto a la representación del orificio de partículas, donde los operadores de creación y Annihilation describen las desplazamientos fuera del estado de referencia usado, como un estado de Hartree – Fock.
...
> [!NOTE]
> Como el valor de `particle_hole_representation` es un objeto de cantidad de matriz dispersa, `unit` `format` se deben especificar las propiedades y.
> Las unidades aceptables se incluyen en la tabla 1.
> La `format` propiedad es obligatoria e indica si los coeficientes de Hamiltonian se especifican como una matriz densa o dispersa.
> En la versión actual, solo se admiten matrices dispersas, con la interpretación de que todos los elementos no especificados son $0 $, pero las versiones futuras pueden agregar compatibilidad con valores adicionales de la `format` propiedad.

### <a name="initial-state-section"></a>Sección de estado inicial ###

El objeto initial_state_suggestion especifica los Estados iniciales de Quantum de interés para el Hamiltonian especificado. Este objeto debe ser una matriz de `state` objetos JSON.

#### <a name="state-object"></a>Objeto de estado ####

Cada Estado representa una superposición de órbitas ocupadas. Cada objeto de Estado debe tener una `label` propiedad que contenga una cadena. Cada objeto de Estado debe tener una `superposition` propiedad que contenga una matriz de Estados de base y sus amplitudes no normalizadas.
...
Cada objeto de conjunto entero puede tener una `basis_set` propiedad.
